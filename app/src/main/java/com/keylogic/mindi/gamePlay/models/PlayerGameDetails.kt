...
data class PlayerGameDetails(
    ...
    var enteredCardList: ArrayList<Card> = ArrayList()
    private var spadeCardList: ArrayList<Card> = ArrayList()
    private var heartCardList: ArrayList<Card> = ArrayList()
    private var clubCardList: ArrayList<Card> = ArrayList()
    private var diamondCardList: ArrayList<Card> = ArrayList()
    var emptySuitList: ArrayList<SuitType> = ArrayList()
    ...
    fun enteredCard(card: Card) {
        for (listCard in cardList) {
            if (listCard.getTag() == card.getTag()) {
                //println("Remove ~~~~~~~~~~~~~~~~ ==> ${getPlayerIdentification()} > ${card.getCard()}")
                cardList.remove(listCard)
                enteredCardList.add(listCard)
                distributeInSeparateList()

                if (GameHelper.currRoundSuitType != SuitType.NONE && GameHelper.currRoundSuitType != card.suit) {
                    emptySuitList.add(GameHelper.currRoundSuitType)
                    if (GameHelper.currRoundSuitType == GameHelper.trumpCardSuit.suit) {
                        isTrumpCardExist = false
                    }
                }
                break
            }
    ...
    fun isSuitExist(suit: SuitType = GameHelper.currRoundSuitType): Boolean {
    ...
    fun convertJsonToCardList(json: String): ArrayList<Card> {
        val gson = Gson()
        val type = object : TypeToken<ArrayList<Card>>() {}.type
        return gson.fromJson(json, type)
    }

    fun convertCardListToJson(cardList: ArrayList<Card>): String {
        val gson = Gson()
